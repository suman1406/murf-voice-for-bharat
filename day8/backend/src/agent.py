...
logger = logging.getLogger("krishivani-day8-agent")
...
4. KNOW WHEN TO ASK FOR HUMAN HELP: Identify situations where an issue requires human expert intervention (Kisan Call Center / Senior Agricultural Specialist):
   - Situation A: The market/weather data is missing, old, or unavailable.
   - Situation B: The farmer reports a serious or severe crop problem (e.g., major pest infestation like Pink Bollworm or Locusts, severe disease outbreak, severe crop loss/damage due to hail/flood, emergency subsidy/insurance dispute).
   - Situation C: The caller explicitly asks to speak with a human expert or specialist.
...
@server.rtc_session(agent_name="krishivani-day8-agent")
async def my_agent(ctx: JobContext):
    ...
    try:
        ...
        success_tools = {"lookup_mandi_rates", "lookup_weather", "create_escalation"}
